...
rollout-s3: $(LOG_DIR)/s3-$(S2_VER).repo.log  cluster/s3-dpl-$(S2_VER).yaml
	$(KC) -n $(APP_NS) apply -f cluster/s3-dpl-$(S2_VER).yaml | tee $(LOG_DIR)/rollout-s2.log
	$(KC) rollout -n $(APP_NS) restart deployment/cmpt756s2-$(S2_VER) | tee -a $(LOG_DIR)/rollout-s2.log
...
clean:
	/bin/rm -f $(LOG_DIR)/{s1,s2,s3,db,gw,monvs,dynamodb}*.log $(LOG_DIR)/rollout*.log
...
loader: dynamodb-init $(LOG_DIR)/loader.repo.log cluster/loader.yaml
	...
	tools/build-configmap.sh gatling/resources/song_info.csv cluster/song_list.yaml | kubectl -n $(APP_NS) apply -f -
...
$(LOG_DIR)/dynamodb-init.log: cluster/cloudformationdynamodb.json
	...
	$(AWS) cloudformation create-stack --stack-name db-foo --template-body file://$< || true | tee $(LOG_DIR)/dynamodb-init.log
...
#
dynamodb-clean:
	$(AWS) cloudformation delete-stack --stack-name db-foo || true | tee $(LOG_DIR)/dynamodb-clean.log
...
s2: rollout-s2 cluster/s2-svc.yaml cluster/s2-sm.yaml cluster/s2-vs-canary.yaml
